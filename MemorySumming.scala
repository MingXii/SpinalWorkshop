  ...
  // TODO define IO
    val io = new Bundle {
      val wr = new Bundle{
        ...
        val data = in UInt(wordWidth bits)
      }
      val rd = new Bundle{
        val clk = in Bool()
        val en = in Bool()
        val addr = in UInt(addressWidth bits)
        val data = out UInt(wordWidth bits)
      }
    }
...
case class MemorySumming(writeClock : ClockDomain,sumClock : ClockDomain) extends Component {
  ...
  val sumArea = new ClockingArea(sumClock){
    ...
    val rd_addr = Counter(8 bits)
    val rd_en = Reg(Bool()) init(False)
    val sum = Reg(UInt(16 bits)) init(0)
    val rd_valid = RegNext(rd_en) init(False)

    ram.io.rd.en := rd_en
    ram.io.rd.addr := rd_addr

    when(io.sum.start){
      rd_en := True
    }.elsewhen(rd_addr.willOverflow){
      rd_en := False
    }.otherwise{
      rd_en := rd_en
    }
    ...
    when(rd_valid) {
      sum := sum + ram.io.rd.data
    }.otherwise{
      sum := 0
    }
    io.sum.done := False
    io.sum.done.setWhen(rd_valid.fall(False))
    io.sum.value := sum
  }
}
/*
